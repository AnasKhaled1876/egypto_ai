...
class GetQuickPromptsResponse {
  ...
  final bool status;
  GetQuickPromptsResponse({
    required this.data,
    required this.status,
  });

  GetQuickPromptsResponse copyWith({
    List<QuickPrompt>? data,
    bool? status,
  }) {
  ...
  factory GetQuickPromptsResponse.fromMap(Map<String, dynamic> map) {
    return GetQuickPromptsResponse(
      data: List<QuickPrompt>.from(map['data']?.map((x) => QuickPrompt.fromMap(x))),
  ...
  factory GetQuickPromptsResponse.fromJson(String source) => GetQuickPromptsResponse.fromMap(json.decode(source));
